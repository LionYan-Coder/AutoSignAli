 ...
 * @Last Modified by: TonyJiangWJ
 * @Last Modified time: 2022-12-07 21:59:47
 * @Description: 
 */
"ui";
...
var indexFilePath = "file://" + mainScriptPath + "/vue_configs/index.html";
var loadingFilePath = "file://" + mainScriptPath + "/vue_configs/loading.html";
var errorFilePath = "file://" + mainScriptPath + "/vue_configs/error.html";
...
prepareWebView(ui.loadingWebview, {
  ...
  onPageFinished: function onPageFinished() {
    var getLocalVersion = function getLocalVersion() {
      var mainPath = FileUtils.getCurrentWorkPath();
      var versionFile = files.join(mainPath, 'version.json');
      var projectFile = files.join(mainPath, 'project.json');
      var versionName = '';
...
/**/
var params = ui.loadingWebview.getLayoutParams();
...
postMessageToWebView = prepareWebView(ui.webview, {
  ...
  // 延迟注册
  bridgeHandler: function bridgeHandler() {
    return bridgeHandlerBuilder(postMessageToWebView);
  },
  onPageFinished: function onPageFinished() {
    log('页面加载完毕');
    registerSensors();
    setTimeout(function () {
      ...
      setTimeout(function () {
        ...
        postMessageToWebView({
          ...
          data: {
            height: ui.loadingWebview.getHeight(),
            width: ui.webview.getWidth()
          }
        });
      }, 100);
...
var distanceSensor = null;
function registerSensors() {
  gravitySensor = sensors.register('gravity', sensors.delay.ui);
  if (gravitySensor) {
    gravitySensor.on('change', function (event, x, y, z) {
      postMessageToWebView({
        functionName: 'gravitySensorChange',
        data: {
          x: x,
          y: y,
          z: z
  ...
  if (distanceSensor) {
    distanceSensor.on('change', function (event, d) {
      postMessageToWebView({
        functionName: 'distanceSensorChange',
        data: {
          distance: d
        }
      });
