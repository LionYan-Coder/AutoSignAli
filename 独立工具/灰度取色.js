...
/***/ "./src/lib/prototype/MlkitOcrUtil.js":
/*!*******************************************!*\
  !*** ./src/lib/prototype/MlkitOcrUtil.js ***!
  \*******************************************/
/***/ ((module, __unused_webpack_exports, __webpack_require__) => {
...
var LogUtils = __webpack_require__(/*! @/lib/prototype/LogUtils */ "./src/lib/prototype/LogUtils.js");
var commonFunctions = __webpack_require__(/*! @/lib/prototype/CommonFunction */ "./src/lib/prototype/CommonFunction.js");
var autoJsSupport = false;
if (typeof $mlKitOcr !== 'undefined') {
  autoJsSupport = true;
}
function MlkitOCR() {
  var pluginOcr = autoJsSupport ? null : function () {
    try {
...
MlkitOCR.prototype.recognize = function (img, region) {
  ...
  var start = new Date();
  var ocrResults = this.ocr.detect(img, {
    region: region
  }).filter(function (v) {
  ...
  return ocrResults.map(function (v) {
    return v.label;
  }).join('\n');
};
...
MlkitOCR.prototype.recognizeWithBounds = function (img, region, regex) {
  ...
  var start = new Date();
  var resultLines = this.ocr.detect(img, {
    region: region
  }).filter(function (v) {
    return v.confidence > 0.5;
  });
  var result = resultLines.map(function (line) {
...
/***/ "./src/lib/prototype/PaddleOcrUtil.js":
/*!********************************************!*\
  !*** ./src/lib/prototype/PaddleOcrUtil.js ***!
  \********************************************/
/***/ ((module, __unused_webpack_exports, __webpack_require__) => {
...
PaddleOcrUtil.prototype.init = function () {
  ...
  var loading = threads.disposable();
  var _this = this;
  // 建议在新线程中初始化模型
  threads.start(function () {
...
PaddleOcrUtil.prototype.initWithCustomizeModel = function (options) {
  ...
  threads.start(function () {
    ...
    var modelPath = options.modelPath ? files.path(options.modelPath) : 'models/ocr_v2_for_cpu'; // 指定自定义模型路径
    var labelPath = options.labelPath ? files.path(options.labelPath) : 'labels/ppocr_keys_v1.txt'; // 指定自定义label路径
...
PaddleOcrUtil.prototype.ensureModelLoaded = function () {
  var _this = this;
  var ocrResults = runtime.bridges.bridges.toArray(_this.predictor.runOcr(images.fromBase64(check_model_base64).getBitmap()));
  var checkResult = ocrResults.map(function (v) {
    return v.label;
  }).join('');
  var limit = 3;
  var loading = threads.disposable();
  threads.start(function () {
    while (checkResult != '测试' && limit-- > 0) {
      ...
      checkResult = ocrResults.map(function (v) {
        return v.label;
      }).join('');
    }
    if (checkResult != '测试') {
...
PaddleOcrUtil.prototype.recognize = function (img, region) {
  ...
  if (this.enabled) {
    ...
    if (region) {
      var imgMat = new Mat(img.mat, buildRegion(region, img));
    ...
    start = new Date();
    var ocrResults = runtime.bridges.bridges.toArray(this.predictor.runOcr(bitmap)).filter(function (v) {
      return v.confidence > 0.5;
    });
    LogUtils.debugInfo(['paddle识别文本耗时：{}ms', new Date() - start]);
    LogUtils.debugInfo(['paddle识别文本信息：{}', JSON.stringify(ocrResults)]);
    bitmapToRelease.forEach(function (bitmap) {
      return bitmap.recycle();
    });
    return ocrResults.sort(function (a, b) {
...
PaddleOcrUtil.prototype.recognizeWithBounds = function (img, region, regex) {
  ...
  if (this.enabled) {
    ...
    LogUtils.debugInfo(['图片转换耗时：{}ms', new Date() - start]);
    start = new Date();
    var result = runtime.bridges.bridges.toArray(this.predictor.runOcr(bitmap)).filter(function (v) {
      return v.confidence > 0.5;
    });
    LogUtils.debugInfo(['paddle识别文本耗时：{}ms', new Date() - start]);
    if (regex) {
    ...
    if (region && region.length > 1) {
      result.forEach(function (r) {
        return r.bounds.offset(region[0], region[1]);
      });
    }
    bitmapToRelease.forEach(function (bitmap) {
      return bitmap.recycle();
...
var fileUtils = __webpack_require__(/*! @/lib/prototype/FileUtils */ "./src/lib/prototype/FileUtils.js");
var formatDate = __webpack_require__(/*! @/lib/DateUtil.js */ "./src/lib/DateUtil.js");
var paddleOcr = __webpack_require__(/*! @/lib/prototype/PaddleOcrUtil */ "./src/lib/prototype/PaddleOcrUtil.js");
var mlkitOcr = __webpack_require__(/*! @/lib/prototype/MlkitOcrUtil */ "./src/lib/prototype/MlkitOcrUtil.js");
// 默认以优先级加载mlkit插件 无插件时使用paddleOcr
var localOcr = null;
var usePaddle = config.local_ocr_priority == 'paddle';
var currentPath = fileUtils.getCurrentWorkPath();
// 强制指定为paddleOcr
if (usePaddle) {
  localOcr = paddleOcr;
} else {
...
var _resourceMonitor = __webpack_require__(/*! @/lib/ResourceMonitor.js */ "./src/lib/ResourceMonitor.js")(runtime, __webpack_require__.g);
...
canvasWindowCtrl.setClick(function () {
  ...
  threads.start(function () {
    ...
    // 将mini按钮移动到屏幕外再贴边
    miniWindowCtrl.windowMoving({
      from: canvasMove.to,
      to: {
        X: canvasMove.to.X + 100,
        Y: canvasMove.to.Y + canvasWindow.getHeight() / 2
      }
    });
...
canvasWindow.btnCapture.click(function () {
  threads.start(function () {
    ...
    sleep(100);
    captureImage = captureScreen();
    _resourceMonitor.delayRecycle(originalImg);
    _resourceMonitor.delayRecycle(grayImg);
    originalImg = images.copy(captureImage, true);
...
canvasWindow.cutOrPoint.on('click', function () {
  threads.start(function () {
    if (cutMode) {
      if (positions && positions.length === 4) {
        ...
        var base64Str = images.toBase64(clipImg);
        setClip(base64Str);
        log('base64:' + base64Str);
        var filePath = 'logs/base64/' + formatDate(new Date(), 'yyyyMMdd/HHmmss.log');
        var savePath = currentPath + '/' + filePath;
...
canvasWindow.toggleOcr.on('click', function () {
  usePaddle = !usePaddle;
  ui.run(function () {
    canvasWindow.toggleOcr.setText(usePaddle ? '切换为Ml-Kit' : '切换为Paddle');
  });
...
canvasWindow.recognizeText.on('click', function () {
  threads.start(function () {
    if (cutMode) {
      ...
      if (positions && positions.length === 4) {
        ...
        if (clipImg.getHeight() < 80) {
          var scale = 80 / clipImg.getHeight();
          var size = [clipImg.getWidth() * scale, 80];
          clipImg = images.resize(clipImg, size);
          log('缩放图片大小：' + clipImg.getWidth() + ',' + clipImg.getHeight());
        }
        var result = localOcr.recognize(clipImg);
        log('识别文本:' + result);
        setClip(result);
        toastLog('识别文本已复制');
        ui.post(function () {
...
canvasWindow.openFile.on('click', function () {
  threads.start(function () {
...
canvasWindow.btnClose.on("click", function () {
  exit();
});
var paint = new Paint();
...
canvasWindow.previewCanvas.on("draw", function (canvas) {
  ...
  // 复制一份 避免闪退
  var forDrawImg = tryCopy(previewImage);
  if (!forDrawImg) {
    return;
  }
  canvas.drawARGB(255, 0, 0, 0);
  var scaleH = canvas.getHeight() / forDrawImg.getHeight();
  var scaleW = canvas.getWidth() / forDrawImg.getWidth();
...
function getRealRegion(positions, data, img) {
  var scaledPositions = [positions[0] - data.translate.x, positions[1] - data.translate.y, positions[2] - data.translate.x, positions[3] - data.translate.y].map(function (v) {
    return v / data.scale;
  });
  var tmp;
...
function convertAndClip(positions, data, img) {
  var _getRealRegion = getRealRegion(positions, data, img),
    left = _getRealRegion.left,
    right = _getRealRegion.right,
    top = _getRealRegion.top,
    bottom = _getRealRegion.bottom,
    width = _getRealRegion.width,
    height = _getRealRegion.height;
      ...
      switch (event.getAction() <= 2 ? event.getAction() : Math.abs(event.getAction() % 2 - 1)) {
        ...
        case event.ACTION_MOVE:
          if (pointerCount == 1) {
            ...
            _resourceMonitor.delayRecycle(previewImage);
...
function FloatyController(window, windowMoveId, isCanvasWinow) {
  ...
  this.setMove = function (fun) {
    fun = fun || function () {};
    _this.move = fun;
  };
  this.setLongClick = function (fun, threshold) {
    fun = fun || function () {};
    _this.LongClick = fun;
    if (parseInt(threshold)) {
      _this.longClickThreshold = parseInt(threshold) / 50;
    }
  };
  setInterval(function () {
    if (context.resources.configuration.orientation != _this.orientation) {
      _this.orientation = context.resources.configuration.orientation;
      _this.width = _this.orientation == 1 ? device_width : device_height;
      _this.height = _this.orientation == 2 ? device_width : device_height;
      var xy = pointRangeBack(window.getX(), window.getY(), _this.moveRange(window));
      _this.windowMoving([[window.getX(), window.getY()], [xy.x, xy.y]]);
    }
  }, 100);
  this.windowStartX = 0;
  this.windowStartY = 0;
  ...
  setInterval(function () {
    if (_this.eventKeep) {
      _this.keepTime++;
      if (!_this.eventMoving && _this.keepTime > _this.longClickThreshold) {
        //非移动且按下时长超过1秒判断为长按
  ...
  if (windowMoveId) {
    windowMoveId.setOnTouchListener(new View.OnTouchListener(function (view, event) {
      _this.move(view, event);
      try {
        switch (event.getAction()) {
          case event.ACTION_DOWN:
            _this.eventStartX = event.getRawX();
            _this.eventStartY = event.getRawY();
            _this.windowStartX = window.getX();
            _this.windowStartY = window.getY();
          ...
          case event.ACTION_MOVE:
            ...
            if (_this.eventMoving && _this.eventKeep) {
              ui.post(function () {
                window.setPosition(_this.windowStartX + sx, _this.windowStartY + sy);
              });
            }
            break;
          case event.ACTION_UP:
            if (!_this.eventMoving && _this.eventKeep && _this.keepTime < 7) {
              _this.Click();
            }
            _this.eventKeep = false;
            _this.keepTime = 0;
            if (_this.eventMoving) {
              if (_this.isAutoIntScreen) {
                threads.start(new java.lang.Runnable(function () {
                  _this.windowMoving(_this.intoScreen());
                  if (!isCanvasWinow) {
                    // mini 悬浮窗自动贴边
                    _this.windowMoving(_this.toScreenEdge());
                  }
                }));
              } else {
                threads.start(new java.lang.Runnable(function () {
                  _this.windowMoving(_this.viewIntoScreen());
                }));
              }
  ...
  this.moveRange = function (win, view) {
    //返回悬浮窗的坐标范围。
    var border = 36,
      //悬浮窗的隐形边矩
      barHeight = 66,
    ...
    if (!isCanvasWinow) {
      if (view) {
        return convertArrayToReadable([[-view.getX(), -view.getY()], [_this.width - (view.getX() + view.getWidth()), _this.height - (view.getY() + view.getHeight()) - barHeight - border - bottomHeight]]);
      } else {
        return convertArrayToReadable([[0, 0], [_this.width - win.getWidth() + border * 2, _this.height - win.getHeight() - barHeight + border * 2 - bottomHeight]]);
      }
    } else {
      if (view) {
        return convertArrayToReadable([[-view.getX(), barHeight - view.getY()], [_this.width - (view.getX() + view.getWidth()), _this.height - (view.getY() + view.getHeight()) - bottomHeight]]);
      } else {
        return convertArrayToReadable([[0, barHeight], [_this.width - win.getWidth(), _this.height - win.getHeight() - bottomHeight]]);
      }
    }
  };
  this.windowMoving = function (moveInfo) {
    //移动悬浮窗的动画效果。
  ...
  this.windowSetPosition = function (x, y) {
    window.setPosition(x, y);
  };
  this.outScreen = function () {
    //算出最短的距离到达屏幕之外。
    var moveRangeInfo = _this.moveRange(window);
    var x = window.getX(),
      y = window.getY();
    var centerX = x + window.getWidth() / 2;
    var toLeft = centerX < _this.width / 2;
    // 如果在左侧 向左移动（x+width） 右侧直接移动到屏幕外
    var targetX = toLeft ? -(x + window.getWidth()) : _this.width + 10;
    return convertArrayToReadable([[x, y], [targetX, y]]);
  };
  this.toScreenEdge = function () {
    //返回到屏幕边缘的坐标。
    var x = window.getX(),
      y = window.getY();
    ...
    // 左侧贴坐标，右侧贴右边
    var cx = centerX < _this.width / 2 ? -window.getWidth() / 3 : _this.width - window.getWidth() / 2;
    return convertArrayToReadable([[x, y], [cx, y]]);
  };
  this.centerXY = function (point) {
    //返回距离中心位置的一个方形两个坐标。
  ...
  this.intoScreen = function () {
    //当悬浮超出屏幕之外之后进入的坐标。
    var point = pointRangeBack(window.getX(), window.getY(), _this.moveRange(window));
    return convertArrayToReadable([[window.getX(), window.getY()], [point.x, point.y]]);
  };
  this.viewIntoScreen = function () {
    //当悬浮超出屏幕之外之后进入的坐标。
    var point = pointRangeBack(window.getX(), window.getY(), _this.moveRange(window, windowMoveId));
    return convertArrayToReadable([[window.getX(), window.getY()], [point.x, point.y]]);
  };
  threads.start(new java.lang.Runnable(function () {
    _this.windowMoving(_this.intoScreen());
