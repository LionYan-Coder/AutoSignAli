 ...
 * @Date: 2019-11-05 09:12:00
 * @Last Modified by: TonyJiangWJ
 * @Last Modified time: 2022-12-16 20:23:40
 * @Description: 
 */
var _require = __webpack_require__(/*! @/secureConfig.js */ "./src/secureConfig.js"),
  _config = _require.config,
  _storageName = _require.storageName;
var automator = __webpack_require__(/*! @/lib/prototype/Automator */ "./src/lib/prototype/Automator.js");
var _require2 = __webpack_require__(/*! @/lib/prototype/LogUtils */ "./src/lib/prototype/LogUtils.js"),
  infoLog = _require2.infoLog,
  logInfo = _require2.logInfo,
...
var FileUtils = __webpack_require__(/*! @/lib/prototype/FileUtils */ "./src/lib/prototype/FileUtils.js");
var _commonFunctions = __webpack_require__(/*! @/lib/prototype/CommonFunction */ "./src/lib/prototype/CommonFunction.js");
var _widgetUtils = __webpack_require__(/*! @/lib/prototype/WidgetUtils */ "./src/lib/prototype/WidgetUtils.js");
var _runningQueueDispatcher = __webpack_require__(/*! @/lib/prototype/RunningQueueDispatcher */ "./src/lib/prototype/RunningQueueDispatcher.js");
var ExternalUnlockDevice = files.exists(FileUtils.getCurrentWorkPath() + '/extends/ExternalUnlockDevice.js') ? eval('require')(FileUtils.getCurrentWorkPath() + '/extends/ExternalUnlockDevice.js') : null;
if (ExternalUnlockDevice) {
  logInfo('使用自定义解锁模块');
} else {
  logInfo('使用内置解锁模块');
}
var NORMAL_DEVICE = function NORMAL_DEVICE(BaseUnlocker) {
  BaseUnlocker.call(this);
  this.DEVICE_TYPE = {
    VIVO: 'vivo',
    COLOROS: 'coloros',
    NORMAL: 'normal'
  ...
  this.unlock_pattern = function (password) {
    ...
    var lockBounds = id('com.android.systemui:id/lockPatternView').findOne(_config.timeout_findOne).bounds();
    var boxWidth = (lockBounds.right - lockBounds.left) / 3;
    var boxHeight = (lockBounds.bottom - lockBounds.top) / 3;
    var positions = password.split('').map(function (p) {
...
function Unlocker() {
  var _km = context.getSystemService(context.KEYGUARD_SERVICE);
  this.relock = false;
  this.reTry = 0;
  ...
  this.wakeup = function () {
    if (this.relock && _config.auto_set_brightness && !_config.resetBrightness) {
      ...
      _config.resetBrightness = function () {
  ...
  this.suspendOnAlarm = function () {
    if (_config.suspend_on_alarm_clock) {
      var alarmContent = _widgetUtils.widgetGetOne(_config.suspend_alarm_content || '滑动关闭闹钟', 1000, true, true);
      if (alarmContent) {
  ...
  this.run_unlock = function () {
    ...
    if (_config.check_device_posture) {
      var sensorInfo = _commonFunctions.getDistanceAndGravity(1000);
      if (sensorInfo.z < (_config.posture_threshold_z || 6) && (!_config.check_distance || sensorInfo.distance < 4)) {
        _commonFunctions.setUpAutoStart(5);
...
var MyDevice = ExternalUnlockDevice || NORMAL_DEVICE;
MyDevice.prototype = Object.create(Unlocker.prototype);
MyDevice.prototype.constructor = MyDevice;
var _unlocker = new MyDevice(Unlocker);
module.exports = {
  exec: function exec() {
    _unlocker.reTry = 0;
  ...
  needRelock: function needRelock() {
    logInfo('是否需要重新锁定屏幕：' + _unlocker.relock);
    return _unlocker.relock;
  },
  saveNeedRelock: function saveNeedRelock(notRelock) {
...
/***/ "./src/lib/prototype/CallStateListener.js":
/*!************************************************!*\
  !*** ./src/lib/prototype/CallStateListener.js ***!
  \************************************************/
/***/ ((module, __unused_webpack_exports, __webpack_require__) => {
 ...
 * @Last Modified by: TonyJiangWJ
 * @Last Modified time: 2022-12-07 21:10:14
...
function CallStateListener() {
  var telephonyManager = context.getSystemService(Context.TELEPHONY_SERVICE);
  this.isReceiverRegistered = false;
  var that = this;
  var onReceiveCallStateChanged = function onReceiveCallStateChanged(context, intent) {
    logUtils.debugInfo('添加通话状态监听器');
    var currentState = that.getCurrentCallState();
  ...
  var PHONE_STATE_RECEIVER = commonFunctions.createInterfaceOrJavaAdapter(BroadcastReceiver, {
  ...
  this.exitIfNotIdle = function () {
    if (!callStateListener.isIdle()) {
      var callState = callStateListener.getCurrentCallState();
...
var _require2 = __webpack_require__(/*! @/lib/prototype/LogUtils */ "./src/lib/prototype/LogUtils.js"),
  logInfo = _require2.logInfo,
  main_errorInfo = _require2.errorInfo,
  main_warnInfo = _require2.warnInfo,
  main_debugInfo = _require2.debugInfo,
  infoLog = _require2.infoLog,
  debugForDev = _require2.debugForDev,
  flushAllLogs = _require2.flushAllLogs;
var FloatyInstance = __webpack_require__(/*! @/lib/prototype/FloatyUtil */ "./src/lib/prototype/FloatyUtil.js");
var main_commonFunctions = __webpack_require__(/*! @/lib/prototype/CommonFunction */ "./src/lib/prototype/CommonFunction.js");
main_commonFunctions.delayIfBatteryLow();
var main_callStateListener = !config.is_pro && config.enable_call_state_control ? __webpack_require__(/*! @/lib/prototype/CallStateListener */ "./src/lib/prototype/CallStateListener.js") : {
  exitIfNotIdle: function exitIfNotIdle() {}
};
// 用于代理图片资源，请勿移除 否则需要手动添加recycle代码
var main_resourceMonitor = __webpack_require__(/*! @/lib/ResourceMonitor.js */ "./src/lib/ResourceMonitor.js")(runtime, __webpack_require__.g);
var unlocker = __webpack_require__(/*! @/lib/Unlock.js */ "./src/lib/Unlock.js");
var automator = __webpack_require__(/*! @/lib/prototype/Automator */ "./src/lib/prototype/Automator.js");
var mainExecutor = __webpack_require__(/*! @/core/MainExecutor.js */ "./src/core/MainExecutor.js");
main_callStateListener.exitIfNotIdle();
...
main_commonFunctions.killDuplicateScript();
// 注册自动移除运行中任务
main_commonFunctions.registerOnEngineRemoved(function () {
  config.resetBrightness && config.resetBrightness();
  events.removeAllListeners();
  events.recycle();
  main_debugInfo('校验并移除已加载的dex');
  resolver();
  flushAllLogs();
  // 针对免费版内存主动释放，Pro版不需要
  main_commonFunctions.reduceConsoleLogs();
  // 移除运行中任务
...
if (!main_commonFunctions.ensureAccessibilityEnabled()) {
  main_errorInfo('获取无障碍权限失败');
  exit();
}
main_commonFunctions.markExtendSuccess();
logInfo('---前置校验完成;启动系统--->>>>');
// 打印运行环境信息
if (files.exists('version.json')) {
  var content = JSON.parse(files.read('version.json'));
  logInfo(['版本信息：{} nodeId:{}', content.version, content.nodeId]);
} else if (files.exists('project.json')) {
  var _content = JSON.parse(files.read('project.json'));
  logInfo(['版本信息：{}', _content.versionName]);
} else {
  logInfo('无法获取脚本版本信息');
}
...
try {
  ...
  if (!config.forceStop) {
    main_errorInfo('解锁发生异常, 三分钟后重新开始' + e);
    main_commonFunctions.printExceptionStack(e);
    main_commonFunctions.setUpAutoStart(3);
...
// 请求截图权限
var executeArguments = engines.myEngine().execArgv;
main_debugInfo(['启动参数：{}', JSON.stringify(executeArguments)]);
// 定时启动的任务, 将截图权限滞后请求
if (!executeArguments.intent || executeArguments.executeByDispatcher) {
  main_commonFunctions.requestScreenCaptureOrRestart();
  main_commonFunctions.ensureDeviceSizeValid();
}
// 初始化悬浮窗
...
// 自动设置刘海偏移量
main_commonFunctions.autoSetUpBangOffset();
/************************
 * 主程序
 ***********************/
main_commonFunctions.showDialogAndWait(true);
main_commonFunctions.listenDelayStart();
...
if (config.develop_mode) {
  ...
  try {
    ...
    main_commonFunctions.setUpAutoStart(1);
    main_errorInfo('执行异常, 1分钟后重新开始' + e);
    main_commonFunctions.printExceptionStack(e);
  }
}
if (config.auto_lock === true && unlocker.needRelock() === true) {
  main_debugInfo('重新锁定屏幕');
