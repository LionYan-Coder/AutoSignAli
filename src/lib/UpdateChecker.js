...
module.exports = (() => {
  ...
  BaseDownloader.prototype.basePrepareDownloader = function (downloader, targetOutputDir) {
    ...
    this.downloadingExecutor = function (backup) {
      ...
      // 覆盖新的dex到lib下
      let copy_result = files.copy(targetOutputDir + '/resources/for_update/download.dex', targetOutputDir + '/dex/download.dex')
      ...
      if (files.exists(outdate_file_path)) {
        ...
        let outdateFiles = eval('require')(outdate_file_path)
  ...
  BaseDownloader.prototype.init = function () {
    ...
    resolver()
    let dexPath = rootPath + '/dex/download.dex'
